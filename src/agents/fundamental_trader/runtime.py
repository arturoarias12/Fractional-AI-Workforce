"""Standalone runtime and LangGraph-compatible adapter for Fundamental Trader.

Mirrors ``quant_trader.runtime`` / ``technical_trader.runtime`` so the
production graph can wire in whichever traders are ready without
...
class FundamentalTraderRuntime:
    """Application boundary for hiring and invoking one Fundamental Trader."""

    def __init__(self, *, agent: FundamentalTraderAgent) -> None:
        self._agent = agent

    @property
    def agent(self) -> FundamentalTraderAgent:
        return self._agent
    ...
    def _validate_mandate(mandate: PMMandate | Mapping[str, Any]) -> PMMandate:
        ...
        if not isinstance(mandate, Mapping):
            raise MandateValidationError(
                "FundamentalTraderRuntime requires a PMMandate or mapping."
            )
        try:
    ...
    def _validate_execution_context(
        *,
        ...
        if not isinstance(execution_context, Mapping):
            raise MandateValidationError(
                "FundamentalTraderRuntime execution_context must be a "
        ...
        try:
            ...
            raise MandateValidationError(
                f"Invalid Fundamental Trader execution context: {exc}"
            ) from exc
    ...
    def _task(
        ...
        lineage = TaskLineage(
            ...
            task_id=(
                ...
                f"{execution_context.round_number}.fundamental.trader"
            ),
...
def make_langgraph_node(
    ...
    *,
    ...
    output_key: str = "fundamental_trader_package",
...
__all__ = ["FundamentalTraderRuntime", "make_langgraph_node"]
